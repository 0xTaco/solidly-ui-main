...
import pairlist from '../pair.json';
...
class Store {
  ...
  /*_getPairInfo = async (web3, account, overridePairs) => {
    try {
      ...
      const ps = await Promise.all(
        pairs.map(async (pair) => {
          try {
            ...
            const token0 = await this.getBaseAsset(pair.token0_address, false, true)
            const token1 = await this.getBaseAsset(pair.token1_address, false, true)
            ...
            pair.balance = BigNumber(balanceOf).div(10**pair.decimals).toFixed(parseInt(pair.decimals))
            pair.totalSupply = BigNumber(totalSupply).div(10**pair.decimals).toFixed(parseInt(pair.decimals))
            pair.reserve0 = BigNumber(reserves[0]).div(10**pair.token0_decimals).toFixed(parseInt(pair.token0_decimals))
            pair.reserve1 = BigNumber(reserves[1]).div(10**pair.token1_decimals).toFixed(parseInt(pair.token1_decimals))
            pair.claimable0 = BigNumber(claimable0).div(10**pair.token0_decimals).toFixed(pair.token0_decimals)
            pair.claimable1 = BigNumber(claimable1).div(10**pair.token1_decimals).toFixed(pair.token1_decimals)
      ...
      const ps1 = await Promise.all(
        ps.map(async (pair) => {
          try {
            if(pair.gauge && pair.gauge_address !== ZERO_ADDRESS) {
              const [ totalSupply, gaugeBalance, gaugeWeight ] = await this._tryGetGaugeBalances(web3, pair, account, gaugesContract)

              const bribeContract = new web3.eth.Contract(CONTRACTS.BRIBE_ABI, pair.gauge_bribeAddress)
      ...
      let pairs1 = this.getStore('pairs')
      console.log(pairs1)
 ...
 /* _getBaseAssetInfo = async (web3, account) => {
    try {
      ...
      console.log("basebalance:", baseAssets[i].balance = BigNumber(balanceOfs[i]).div(10 ** baseAssets[i].decimals).toFixed(18 /*baseAssets[i].decimals*///))
  ...
  _getBaseAssetInfo = async (web3, account) => {
    try {
      ...
      for (let i = 0; i < baseAssets.length; i++) {
        baseAssets[i].balance = BigNumber(baseAssetsBalances[i].balanceOf).div(10 ** Number(baseAssets[i].decimals)).toFixed(Number(baseAssets[i].decimals))
